...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_0__["default"])(); // sliders('.feedback-slider-item', 'horizontal', '.main-prev-btn', '.main-next-btn');
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      ...
      resultBlock.textContent = Math.round(sub * 0.7);
    } else {
...
var drop = function drop() {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      input.files = e.dataTransfer.files; // Автоматическая отправка изображения на сервер
...
/* harmony default export */ __webpack_exports__["default"] = (drop); // события которые происходят при перетаскивании элемента в браузере
// drag * 
// dragend *
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      elem.setSelectionRange(pos, pos); //Курсор в конце инпута
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        scroll = calcScroll(); //чтобы модальное окно не "прыгало"
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        windows.forEach(function (item) {
          item.style.display = 'none';
          item.classList.add('animated', 'fadeIn'); //подключение анимации при вызове модального окна
        });
        modal.style.display = "block";
        document.body.style.overflow = "hidden";
        document.body.style.marginRight = "".concat(scroll, "px"); //чтобы модальное окно не "прыгало"
    ...
    close.addEventListener('click', function () {
      ...
      document.body.style.marginRight = "0px"; //чтобы модальное окно не "прыгало"
  ...
  } // Убираем растояние скролла для того, чтобы модальное окно не "прыгало" и не было пркрутки при открытии модального окна
  ...
  function calcScroll() {
    ...
    div.remove();
    return scrollWidth;
  } // Если пользователь долистал страницу до конца и не нажал ни на одну клавишу
...
var scrolling = function scrolling(upSelector) {
  ...
  var links = document.querySelectorAll('[href^="#"]'),
      speed = 0.3; // чем меньше значение, тем выше скорость анимации
  ...
  //         ) {
  //             clearInterval(move);
  //             history.replaceState(history.state, document.title, location.href.replase(/#.*$/g, '') + hash);
  //         }   else {
